import OpenAI from "openai";
import { createClient } from "@supabase/supabase-js";
import { extractInfoFromMessage, hasBasicInfo, toUserAnswers, advancePhase, getInitialState, type ConversationState, type CollectedInfo, type ConversationPhase } from "@/utils/conversationManager";
import { rankCities, type CityRawData, type CityScoreResult } from "@/utils/scoring";
import { rankSchools, type SchoolData, type SchoolScoreResult } from "@/utils/schoolScoring";
import { buildConversationPrompt } from "@/utils/promptBuilder";

export const dynamic = "force-dynamic";

const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

/** Supabaseの複数テーブルを結合して都市スコアリング用の生データを取得する */
async function fetchCitiesRawData(): Promise<CityRawData[]> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );

    const [climateRes, costRes, safetyRes, jobsRes, schoolsRes] = await Promise.all([
      supabase.from('city_climate').select('city_id, city, city_en, sunshine_hours'),
      supabase.from('city_cost_of_living').select('city, rent_1br_city_aud'),
      supabase.from('city_safety').select('city_en, safety_index'),
      supabase.from('city_jobs').select('city_id, score_jobs, score_japanese_jobs'),
      supabase.from('schools').select('city'),
    ]);

    // city_climate を基準に Map を作成
    const map = new Map<string, CityRawData>();
    for (const r of (climateRes.data ?? [])) {
      map.set(r.city_id, {
        city_id: r.city_id, city: r.city, city_en: r.city_en,
        rent_1br_city_aud: null, safety_index: null,
        sunshine_hours: r.sunshine_hours ?? null,
        score_jobs: null, score_japanese_jobs: null, school_count: 0,
      });
    }

    // 生活費（日本語の都市名でマッチ）
    for (const r of (costRes.data ?? [])) {
      for (const v of map.values()) {
        if (v.city === r.city) { v.rent_1br_city_aud = r.rent_1br_city_aud ?? null; }
      }
    }
    // 治安（英語名でマッチ）
    for (const r of (safetyRes.data ?? [])) {
      for (const v of map.values()) {
        if (v.city_en === r.city_en) { v.safety_index = r.safety_index ?? null; }
      }
    }
    // 仕事スコア（city_id でマッチ）
    for (const r of (jobsRes.data ?? [])) {
      const v = map.get(r.city_id);
      if (v) { v.score_jobs = r.score_jobs ?? null; v.score_japanese_jobs = r.score_japanese_jobs ?? null; }
    }
    // 語学学校カウント（日本語の都市名でカウント）
    for (const r of (schoolsRes.data ?? [])) {
      for (const v of map.values()) {
        if (v.city === r.city) v.school_count++;
      }
    }

    return [...map.values()];
  } catch (e) {
    console.error('[fetchCitiesRawData error]', e);
    return [];
  }
}

/** スコアリングが必要なフェーズかどうか */
function needsScoring(phase: ConversationPhase): boolean {
  return ['city', 'school', 'accommodation', 'summary'].includes(phase);
}

async function fetchCityJobsPrompt(): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );
    const { data } = await supabase
      .from('city_jobs')
      .select('city,city_en,indeed_casual_count,score_jobs,score_japanese_jobs,peak_season,farm_nearby,dominant_job_types,summary_ja,hospitality_level,japanese_biz_level')
      .order('score_jobs', { ascending: false });
    if (!data || data.length === 0) return '';

    type JobRow = {
      city: string; city_en: string;
      indeed_casual_count: number | null;
      score_jobs: number | null; score_japanese_jobs: number | null;
      peak_season: string | null; farm_nearby: boolean | null;
      dominant_job_types: string | null; summary_ja: string | null;
      hospitality_level: string | null; japanese_biz_level: string | null;
    };

    const lvl = (v: string | null) => v === 'H' ? '多い' : v === 'M' ? '中程度' : v === 'L' ? '少ない' : '未取得';

    const lines = (data as JobRow[]).map(c => [
      `### ${c.city}（${c.city_en}）`,
      `仕事スコア: ${c.score_jobs ?? '—'}/5 / 日本語求人スコア: ${c.score_japanese_jobs ?? '—'}/5`,
      `Indeedカジュアル求人: 約${c.indeed_casual_count != null ? c.indeed_casual_count.toLocaleString() : '—'}件`,
      `ホスピタリティ求人: ${lvl(c.hospitality_level)} / 日本語対応職場: ${lvl(c.japanese_biz_level)}`,
      c.farm_nearby ? 'ファーム作業（88日条件）: 近郊あり' : '',
      c.peak_season ? `求人ピーク: ${c.peak_season}` : '',
      c.dominant_job_types ? `主な職種: ${c.dominant_job_types}` : '',
      c.summary_ja ? `特徴: ${c.summary_ja}` : '',
    ].filter(Boolean).join('\n')).join('\n\n');

    return `\n\n# 都市別仕事市場データ（Indeed 2026年5月実測 + 労働市場調査）\n仕事・求人・働き方に関する質問にはこのデータを使って具体的に案内してください。データはIndeed実測値をベースにしており、Seek・Gumtree等のデータは含まれないため実際の総数はさらに多い可能性があります。\n\n${lines}`;
  } catch {
    return '';
  }
}

async function fetchCityClimatePrompt(): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );
    const { data } = await supabase
      .from('city_climate')
      .select('city,city_en,temp_avg_c,temp_summer_c,temp_winter_c,rainfall_mm,sunshine_hours,climate_type,koppen,summary')
      .order('city_id');
    if (!data || data.length === 0) return '';

    type ClimateRow = {
      city: string; city_en: string;
      temp_avg_c: number | null; temp_summer_c: number | null; temp_winter_c: number | null;
      rainfall_mm: number | null; sunshine_hours: number | null;
      climate_type: string | null; koppen: string | null; summary: string | null;
    };

    const lines = (data as ClimateRow[]).map(c => [
      `### ${c.city}（${c.city_en}）`,
      `気候区分: ${c.climate_type ?? '未取得'}${c.koppen ? `（ケッペン: ${c.koppen}）` : ''}`,
      `年間平均気温: ${c.temp_avg_c ?? '未取得'}°C / 夏の平均最高: ${c.temp_summer_c ?? '未取得'}°C / 冬の平均最低: ${c.temp_winter_c ?? '未取得'}°C`,
      `年間降水量: ${c.rainfall_mm != null ? `${c.rainfall_mm}mm` : '未取得'} / 年間日照時間: ${c.sunshine_hours != null ? `${c.sunshine_hours}時間` : '未取得'}`,
      c.summary ? `特徴: ${c.summary}` : '',
    ].filter(Boolean).join('\n')).join('\n\n');

    return `\n\n# 都市別気候データ（Wikipedia 2026年5月取得）\n気候・天気・季節に関する質問にはこのデータを使って具体的に案内してください。\n\n${lines}`;
  } catch {
    return '';
  }
}

async function fetchCitySafetyPrompt(): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );
    const { data } = await supabase
      .from('city_safety')
      .select('city,city_en,safety_index,crime_index,safety_daytime,safety_nighttime,problem_drugs,problem_property_crime,problem_violent_crime,crime_increasing_5yr,summary_ja,notes')
      .order('safety_index', { ascending: false });
    if (!data || data.length === 0) return '';

    type SafetyRow = {
      city: string; city_en: string;
      safety_index: number | null; crime_index: number | null;
      safety_daytime: number | null; safety_nighttime: number | null;
      problem_drugs: number | null; problem_property_crime: number | null;
      problem_violent_crime: number | null; crime_increasing_5yr: number | null;
      summary_ja: string | null; notes: string | null;
    };

    const level = (v: number | null) => {
      if (v == null) return '未取得';
      if (v >= 60) return `${v}（高）`;
      if (v >= 35) return `${v}（中）`;
      return `${v}（低）`;
    };

    const lines = (data as SafetyRow[]).map(c => [
      `### ${c.city}（${c.city_en}）`,
      `安全指数: ${c.safety_index ?? '未取得'} / 犯罪指数: ${c.crime_index ?? '未取得'}`,
      `昼間の安全感: ${level(c.safety_daytime)} / 夜間の安全感: ${level(c.safety_nighttime)}`,
      `薬物問題: ${level(c.problem_drugs)} / 物品犯罪: ${level(c.problem_property_crime)} / 暴力犯罪: ${level(c.problem_violent_crime)}`,
      `近年の犯罪増加感: ${level(c.crime_increasing_5yr)}`,
      c.summary_ja ? `概要: ${c.summary_ja}` : '',
      c.notes ? `備考: ${c.notes}` : '',
    ].filter(Boolean).join('\n')).join('\n\n');

    return `\n\n# 都市別治安データ（Numbeo 2026年4〜5月取得）\n治安・安全性に関する質問にはこのデータを使って具体的に案内してください。スコアは0〜100で、安全指数は高いほど安全、犯罪指数は高いほど危険です。\n\n${lines}`;
  } catch {
    return '';
  }
}

async function fetchVisaInfoPrompt(): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );
    const { data } = await supabase
      .from('visa_info')
      .select('visa_code,visa_name_ja,visa_name_en,target_user_ja,age_restriction,stay_duration,work_restriction,application_fee_aud,required_documents,application_method,second_third_info,notes')
      .order('visa_code');
    if (!data || data.length === 0) return '';

    type VisaRow = {
      visa_code: string;
      visa_name_ja: string;
      visa_name_en: string;
      target_user_ja: string | null;
      age_restriction: string | null;
      stay_duration: string | null;
      work_restriction: string | null;
      application_fee_aud: number | null;
      required_documents: string | null;
      application_method: string | null;
      second_third_info: string | null;
      notes: string | null;
    };

    const lines = (data as VisaRow[]).map(v => [
      `### ${v.visa_name_ja}（${v.visa_name_en} / サブクラス${v.visa_code}）`,
      v.target_user_ja ? `対象者: ${v.target_user_ja}` : '',
      v.age_restriction ? `年齢制限: ${v.age_restriction}` : '',
      v.stay_duration ? `滞在期間: ${v.stay_duration}` : '',
      v.work_restriction ? `就労制限: ${v.work_restriction}` : '',
      v.application_fee_aud != null ? `申請費用: A$${v.application_fee_aud}（約${Math.round(v.application_fee_aud * 95).toLocaleString()}円）` : '',
      v.required_documents ? `必要書類: ${v.required_documents}` : '',
      v.application_method ? `申請方法: ${v.application_method}` : '',
      v.second_third_info ? `セカンド・サード条件: ${v.second_third_info}` : '',
      v.notes ? `注意点: ${v.notes}` : '',
    ].filter(Boolean).join('\n')).join('\n\n');

    return `\n\n# オーストラリア ビザ情報（2026年5月時点）\nビザに関する質問にはこのデータを使って正確に案内してください。費用・条件は変更される場合があるため「目安」として案内し、最新情報は公式サイトで確認するよう伝えてください。\n\n${lines}`;
  } catch {
    return '';
  }
}

async function fetchCostOfLivingPrompt(): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );
    const { data } = await supabase
      .from('city_cost_of_living')
      .select('city,city_en,meal_cheap_aud,meal_midrange_2p_aud,meal_fastfood_aud,coffee_aud,beer_local_aud,milk_1l_aud,eggs_12_aud,chicken_1kg_aud,beef_1kg_aud,rice_1kg_aud,tomato_1kg_aud,apple_1kg_aud,rent_1br_city_aud,rent_1br_suburbs_aud,rent_3br_city_aud,rent_3br_suburbs_aud,transport_one_way_aud,transport_monthly_aud,gas_per_liter_aud,taxi_start_aud,utilities_85sqm_aud,mobile_plan_aud,internet_aud,gym_monthly_aud,movie_ticket_aud,avg_monthly_salary_aud,notes')
      .order('city');
    if (!data || data.length === 0) return '';

    const JPY = 95;
    const f = (v: number | null, suffix = '') =>
      v != null ? `A$${v}（約${Math.round(v * JPY).toLocaleString()}円）${suffix}` : '未取得';

    const lines = (data as Array<Record<string, number | string | null>>).map(c => [
      `### ${c.city}（${c.city_en}）`,
      `外食: 安いレストラン ${f(c.meal_cheap_aud as number | null)} / 中級2人 ${f(c.meal_midrange_2p_aud as number | null)} / ファーストフード ${f(c.meal_fastfood_aud as number | null)} / カフェ ${f(c.coffee_aud as number | null)} / ビール ${f(c.beer_local_aud as number | null)}`,
      `食料品: 牛乳1L ${f(c.milk_1l_aud as number | null)} / 卵12個 ${f(c.eggs_12_aud as number | null)} / 鶏肉1kg ${f(c.chicken_1kg_aud as number | null)} / 牛肉1kg ${f(c.beef_1kg_aud as number | null)} / 米1kg ${f(c.rice_1kg_aud as number | null)}`,
      `家賃/月: 1BR市街地 ${f(c.rent_1br_city_aud as number | null, '/月')} / 1BR郊外 ${f(c.rent_1br_suburbs_aud as number | null, '/月')} / 3BR市街地 ${f(c.rent_3br_city_aud as number | null, '/月')} / 3BR郊外 ${f(c.rent_3br_suburbs_aud as number | null, '/月')}`,
      `交通: 片道 ${f(c.transport_one_way_aud as number | null)} / 月間パス ${f(c.transport_monthly_aud as number | null, '/月')} / ガソリン1L ${f(c.gas_per_liter_aud as number | null)} / タクシー初乗り ${f(c.taxi_start_aud as number | null)}`,
      `光熱費・通信/月: 電気ガス水道（85㎡） ${f(c.utilities_85sqm_aud as number | null, '/月')} / スマホ ${f(c.mobile_plan_aud as number | null, '/月')} / ネット ${f(c.internet_aud as number | null, '/月')}`,
      `娯楽: ジム ${f(c.gym_monthly_aud as number | null, '/月')} / 映画 ${f(c.movie_ticket_aud as number | null)}`,
      `平均月収（税引後）: ${f(c.avg_monthly_salary_aud as number | null, '/月')}`,
      c.notes ? `備考: ${c.notes}` : '',
    ].filter(Boolean).join('\n')).join('\n\n');

    return `\n\n# 都市別生活費データ（Numbeo 2026年5月取得、1AUD≈95円）\n費用シミュレーションや都市比較の質問には、必ず以下の実データを使って具体的な金額をAUDと日本円の両方で案内してください。\n\n${lines}`;
  } catch {
    return '';
  }
}

async function fetchSchoolsPrompt(
  userAnswers?: any,
  targetCity?: string
): Promise<string> {
  try {
    const supabase = createClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
    );

    // 詳細データがある学校（school_id があるもの）を優先的に取得
    const { data: detailedSchools } = await supabase
      .from("schools")
      .select("*")
      .not('school_id', 'is', null)
      .order('created_at', { ascending: false });

    if (detailedSchools && detailedSchools.length > 0) {
      // 詳細データがある学校のプロンプト生成
      const schoolsData = await Promise.all(
        detailedSchools.map(async (school: any) => {
          // コース情報取得
          const { data: courses } = await supabase
            .from('school_courses')
            .select('*')
            .eq('school_id', school.school_id)
            .order('price_per_week_aud', { ascending: true });

          // ラベル取得
          const { data: labels } = await supabase
            .from('school_labels')
            .select('label')
            .eq('school_id', school.school_id);

          // 宿泊施設取得
          const { data: accommodation } = await supabase
            .from('school_accommodation')
            .select('*')
            .eq('school_id', school.school_id);

          return {
            ...school,
            courses: courses || [],
            labels: (labels || []).map((l: any) => l.label),
            accommodation: accommodation?.[0] || null
          };
        })
      );

      // スコアリング実行（ユーザー条件がある場合）
      let rankedSchools = schoolsData;
      if (userAnswers) {
        const scoreResults = rankSchools(schoolsData as SchoolData[], userAnswers, targetCity);
        // スコア情報を元のデータに追加
        rankedSchools = scoreResults.map(result => {
          const school = schoolsData.find(s => s.school_id === result.school_id);
          return {
            ...school,
            _score: result.totalScore,
            _rank: result.rank,
            _matchReasons: result.matchReasons,
            _breakdown: result.breakdown
          };
        });
      }

      const lines = rankedSchools.map((s: any) => {
        const JPY = 95; // 1AUD = 95円

        // 基本情報
        let info = `### ${s.name}${s.name_ja ? `（${s.name_ja}）` : ''}`;

        // スコア情報があれば表示（ユーザー条件に基づくランキング）
        if (s._rank && s._score) {
          info += ` 🎯 [あなたへのおすすめ度: 第${s._rank}位 - 総合${s._score}点/100点]\n`;
          if (s._matchReasons && s._matchReasons.length > 0) {
            info += `✨ マッチ理由: ${s._matchReasons.slice(0, 3).join('、')}\n`;
          }
          if (s._breakdown) {
            const b = s._breakdown;
            info += `   📊 内訳: 予算${b.budget}点 / レベル${b.level}点 / サポート${b.support}点 / 環境${b.environment}点 / 評価${b.rating}点\n`;
          }
        } else {
          info += `\n`;
        }

        info += `📍 都市: ${s.city}（${s.country}）${s.area ? ` - ${s.area}` : ''}\n`;

        // Google評価・写真
        if (s.rating || s.review_count) {
          info += `⭐ Google評価: ${s.rating ?? '-'}/5.0（${s.review_count ?? 0}件のレビュー）\n`;
        }
        if (s.google_photos && s.google_photos.length > 0) {
          info += `📸 学校写真: ${s.google_photos.slice(0, 3).join(', ')}\n`;
        }

        // 料金情報
        if (s.price_min_aud && s.price_max_aud) {
          info += `💰 料金: 週${s.price_min_aud}〜${s.price_max_aud}AUD（約${Math.round(s.price_min_aud * JPY).toLocaleString()}〜${Math.round(s.price_max_aud * JPY).toLocaleString()}円）\n`;
        }
        if (s.enrollment_fee_aud) {
          info += `   入学金: ${s.enrollment_fee_aud}AUD（約${Math.round(s.enrollment_fee_aud * JPY).toLocaleString()}円）\n`;
        }

        // コース情報
        if (s.courses.length > 0) {
          info += `\n📚 コース:\n`;
          s.courses.forEach((c: any) => {
            const price = `${c.price_per_week_aud}AUD/週（約${Math.round(c.price_per_week_aud * JPY).toLocaleString()}円）`;
            const recommended = c.recommended ? ' ⭐推奨' : '';
            const timeSlot = c.time_slot ? ` [${c.time_slot}]` : '';
            info += `   - ${c.name}（${c.name_short}）: ${price}${timeSlot}${recommended}\n`;
            if (c.schedule) info += `     時間: ${c.schedule}\n`;
          });
        }

        // 学校特徴
        const features = [];
        if (s.japanese_ratio != null) features.push(`日本人比率${Math.round(s.japanese_ratio * 100)}%`);
        if (s.total_students) features.push(`生徒数${s.total_students}名`);
        if (s.support_job) features.push('就職サポートあり');
        if (s.support_japanese) features.push('日本語サポートあり');
        if (s.support_accommodation) features.push('住居サポートあり');
        if (s.accredited) features.push('認定校');

        if (features.length > 0) {
          info += `\n✨ 特徴: ${features.join('、')}\n`;
        }

        // ラベル（タグ）
        if (s.labels.length > 0) {
          info += `🏷️ タグ: ${s.labels.join(', ')}\n`;
        }

        // 宿泊施設
        if (s.accommodation) {
          const acc = s.accommodation;
          info += `\n🏠 宿泊施設: ${acc.type}（${acc.room_type}）\n`;
          info += `   料金: ${acc.price_per_week_aud}AUD/週（約${Math.round(acc.price_per_week_aud * JPY).toLocaleString()}円）\n`;
          info += `   食事: ${acc.meal_plan}\n`;
        }

        // 学校概要
        if (s.school_summary) {
          info += `\n📝 概要: ${s.school_summary}\n`;
        }

        return info;
      }).join("\n");

      return `\n\n# 提案可能な語学学校リスト（詳細データあり）

以下の学校は詳細なコース情報・料金データがあります。
学校を提案する際は、ユーザーの条件（予算・目的・サポート・環境）に合わせて具体的に紹介してください。

${lines}

**提案時の重要ルール**：
1. 🎯 **スコアリング情報を最優先**: 学校に「あなたへのおすすめ度」が表示されている場合、そのランキング順に提案すること
2. ✨ **マッチ理由を必ず伝える**: スコアリングの「マッチ理由」を活用して、なぜその学校がおすすめなのかを具体的に説明すること
3. 📊 **内訳を活用**: 予算・レベル・サポート・環境・評価の内訳から、ユーザーの重視ポイントに合った説明をすること
4. 💰 予算重視のユーザーには最安コースを提示すること
5. 🤝 就職サポートが必要なユーザーには support_job: true の学校を優先すること
6. 🌏 日本人比率を気にするユーザーには具体的な数値を伝えること
7. 📚 複数コースがある場合は、ユーザーの目的（一般英語、IELTS、ビジネス等）に合ったコースを提案すること
8. 🌙 働きながら学びたいユーザーには evening（夜間）コースを提案すること
9. ⭐ Google評価が高い学校はその点を強調すること
10. 📸 学校写真があれば「学校の雰囲気を写真で確認できます」と案内すること

上記以外の学校名は作らないこと。`;
    }

    // 詳細データがない場合は基本データのみ
    const { data } = await supabase.from("schools").select("name, city, country, type, website, is_partner");
    if (!data || data.length === 0) return "";
    const lines = data.map((s: { name: string; city: string; type: string; website: string | null; is_partner: boolean }) =>
      `- ${s.name}（${s.city}）: ${s.type}、${s.is_partner ? "提携校" : "非提携校"}`
    ).join("\n");
    return `\n\n# 提案可能な語学学校リスト\n以下の学校をプランニング・提案に使用してください。\n\n${lines}`;
  } catch (error) {
    console.error('[fetchSchoolsPrompt error]', error);
    return "";
  }
}

const SYSTEM_PROMPT = `あなたはAbroのAI留学・ワーキングホリデープランナーです。
役割は、単なるFAQ回答者ではありません。
ユーザーが「自分に合う国・都市・学校・プログラム・ビザ・予算・準備ステップ」を明確にし、
不安を減らしながら、次の行動（比較・保存・相談予約・見積依頼）まで進められるように支援する、
信頼できる留学コンシェルジュです。

# ミッション
ユーザーの希望・予算・期間・英語力・年齢・出発時期・不安を会話の中で自然に整理し、
最適な選択肢を、理由付きで、わかりやすく、押しつけずに提案してください。
常に「回答」ではなく「意思決定を前に進めること」を目的にしてください。

# 最重要方針
このAIは、一般的な留学情報を話すだけのAIではありません。
{{brand_name}} のアプリ・サイトに登録、掲載、記載、投稿されている情報を優先して回答・提案・プランニングを行うAIです。

したがって、すべての回答で以下の優先順位を守ってください。

1. アプリ・サイト内に登録/掲載/投稿されている情報
2. 社内で整理された公式データ、学校データ、都市データ、FAQ、体験談、ガイド、投稿情報
3. 一般的に広く知られている留学・ワーホリの基本知識
4. 情報不足を補うための一般的な参考提案

## 情報利用ルール
- 回答、提案、比較、プランニングは、まずアプリ・サイト内の情報を根拠に行うこと
- アプリ・サイトにある学校、都市、プログラム、投稿、体験談、FAQ、ガイド、費用目安、準備情報があれば、それを優先すること
- アプリ・サイトに存在しない学校名、プラン名、投稿内容を作らないこと
- 情報が不足している場合のみ、一般的な知識で補完してよい
- その場合は、アプリ・サイト内の確定情報と、一般的な参考情報を混同しないこと
- サイト/アプリ上の情報が少ないときは、「現時点の掲載情報ベースではこう考えられます」としたうえで、現実的な仮提案を行うこと
- ユーザーがアプリ/サイトの機能、掲載情報、登録内容、投稿内容について質問した場合は、登録・掲載されている内容に沿って正確に答えること
- 登録情報にないことは断定せず、「現在確認できる情報では」「掲載情報ベースでは」とやわらかく伝えること

# ブランドトーン
- 親身
- 信頼感がある
- 実務的
- 押し売りしない
- 不安を煽らない
- でも曖昧すぎず、ちゃんと方向性を示す
- 「あなたの場合はこう考えるとよいです」と伴走する口調
- 日本語は自然でやさしく、でも幼すぎない
- 初めての海外でも理解しやすい言い回しを使う

# 対応領域
以下の相談に対応してください。

## 1. 留学・ワーキングホリデー全般
- ワーキングホリデーと語学留学の違い
- 目的・予算・期間に合った留学/ワーホリの選び方
- 留学の向き不向き
- 初めての海外でも安心な選択肢
- 社会人・大学生・30代など属性別の考え方
- 英語力が低い場合の現実的な始め方
- 留学前後の全体スケジュール
- 渡航前準備チェックリスト
- 渡航後の生活立ち上げ

## 2. 国・都市情報
初期フェーズではオーストラリアを最優先で詳しく案内してください。

重点都市:
- シドニー
- メルボルン
- ブリスベン
- パース
- ゴールドコースト
- ケアンズ

案内内容:
- 都市の雰囲気
- 生活費の傾向
- 仕事の探しやすさ
- ファームジョブのしやすさ
- 住みやすさ
- 初心者向きかどうか
- 英語環境
- 日本人比率の傾向
- 学校の選びやすさ
- 交通や治安の一般的な傾向

オーストラリア以外の国について質問された場合は、
一般的な比較や考え方は案内してよいですが、
詳細データが不足している場合は断定せず、
「初期フェーズではオーストラリア中心に詳しく案内しています」とやわらかく伝えてください。

## 3. 学校・プログラム情報
- 語学学校の種類
  - 一般英語
  - IELTS対策
  - ケンブリッジ対策
  - ビジネス英語
  - 進学準備
- 学校選びのポイント
  - 立地
  - クラスサイズ
  - 国籍比率
  - 日本人比率
  - 学費
  - サポート体制
  - 雰囲気
- 大学進学
  - ファウンデーション
  - 学士
  - 修士
  - MBA
- 専門学校 / 職業訓練
  - 調理
  - 美容
  - IT
  - 保育
  - ホスピタリティ
  - 看護関連
- 目的に応じた学校タイプの提案

重要:
- このシステムプロンプトの末尾に「提案可能な語学学校リスト」が記載されている場合、それが提案に使える学校の完全なリストです。
- **重要**: 学校を提案するのは、ユーザーが「学校を探している」「学校を教えてほしい」「おすすめの学校は？」など、明確に学校の提案を求めている場合のみです。
- 都市選びや予算相談の段階では、学校名を具体的に出さず、「学校の選択肢は豊富にあります」「ご希望が固まったら具体的にご提案できます」という案内にとどめてください。
- 学校を提案する際は、リストから学校名を具体的に挙げ、「おすすめ理由」とともに提案してください。
- 「○○がお好みとのことなので、△△がぴったりだと思います」という形で、必ず提案理由をセットにすること。
- ユーザーが質問した都市に学校がない場合でも、同じ国の他の都市の学校を「○○にはなりますが、こちらもご提案できます」と紹介してください。
- 架空の学校名は絶対に作らないこと。リストにない学校名は言わない。
- 学校を紹介するときは「〜が登録されています」ではなく「〜をご提案します」「〜が候補になります」という口調を使うこと。

## 4. ビザ・法律・手続き
- ワーキングホリデービザの基本条件
- 学生ビザの基本条件
- セカンドワーホリ
- ファームジョブ
- 就労可能時間の一般的な考え方
- 申請時の必要書類
- 渡航までの手続きの順番

重要:
- ビザ・法律・就労・税務は変更されることがあるため、確定情報のように断言しすぎないこと
- アプリ・サイトに登録されているビザガイドやFAQがある場合は、それを優先して要約・整理すること
- 法律相談・移民アドバイス・税務アドバイスを断定して行わないこと
- 必要に応じて「最新情報は必ず公式機関、大使館、移民局などで確認してください」と明示すること

## 5. 費用・お金の管理
- 都市別の生活費目安
- 家賃・食費・交通費の相場感
- フライト費用の目安
- 安く行くための考え方
- 学費の目安
- 両替・送金サービス
- 保険の考え方
- 節約術
- 初期費用と現地到着後費用の分け方

費用を案内する際は、できるだけ以下に分けてください。
- 渡航前に必要なお金
- 現地到着直後に必要なお金
- 毎月かかる固定費
- 変動しやすい費用
- 節約しやすい項目
- 見落としやすい費用

重要:
- 費用提案は、アプリ・サイトに登録されている費用目安、学校料金、体験談、見積情報、比較データを優先して使うこと
- 情報が少ない場合のみ一般的な相場感で補完すること
- 確定金額のように見せず、目安であることを明確にすること

## 6. 仕事・収入
- ワーホリ中の仕事の探し方
- 仕事探しの始め方
- 履歴書 / CV の考え方
- 最低賃金の見方
- 税番号（例: TFN など）の基本
- アルバイト / カジュアルジョブの種類
- リモートワークやフリーランスとの併用の考え方
- 英語力が低い場合の現実的な仕事戦略

重要:
- アプリ・サイトに体験談、仕事探しガイド、都市別の仕事傾向、投稿情報がある場合はそれを優先すること
- 仕事の採用保証のような表現はしないこと
- 一般論だけでなく、「掲載情報ベースで見ると、こういう傾向があります」と整理すること

## 7. アプリ・サイト掲載情報の案内
ユーザーがアプリ・サイト内の情報について質問した場合は、以下に対応してください。
- 登録されている学校やプログラムの案内
- 投稿されている体験談やガイドの要約
- 保存できる情報や比較できる情報の案内
- 掲載されている都市情報の整理
- 登録済みの準備情報、FAQ、チェックリストの案内
- 記載されている機能や導線の説明

重要:
- サイトやアプリに書かれていることを、ユーザーが理解しやすい言葉に整理して伝えること
- 情報を単に読み上げるのではなく、「要するに何が言えるか」を説明すること
- 複数情報がある場合は、比較しやすいようにまとめること

# 会話の基本方針
あなたの仕事は、一度に情報を大量に並べることではありません。
まずユーザーに合う条件を把握し、
必要なことだけを、順序よく、わかりやすく提案してください。

常に以下を意識してください。
1. ユーザーの不安を減らす
2. 選択肢を整理する
3. 迷いを比較可能な形に変える
4. 次に何をすればいいかを明確にする
5. 回答・提案の根拠を、できるだけアプリ・サイト内情報に置く

# まず確認すべき主要項目
会話の中で自然に、必要に応じて以下を把握してください。
- 目的（英語力向上、海外経験、進学、キャリア、移住準備、海外就労など）
- 予算
- 期間
- 出発時期
- 年齢 / 属性（大学生、社会人、休学、転職前など）
- 英語レベル
- 希望国 / 希望都市
- 仕事重視か学習重視か
- 不安点
- どこまでサポートが必要か
- 一人渡航かどうか
- 海外経験の有無

ただし、毎回すべてを質問しないでください。
すでに会話に出ている情報は繰り返し聞かず、
不足している情報だけを、自然な流れで、優先度の高い順に確認してください。

# 質問の仕方（重要）

## スコアリングのための必須情報収集
最適な都市・学校を提案するには、以下の4つの情報が**必須**です：
1. **目的**（英語力向上・お金を稼ぐ・キャリア・海外体験・永住権）
2. **予算**（100万未満・100〜200万・200万以上）
3. **英語力**（初心者・中級・上級）
4. **重視条件**（生活費・治安・仕事・気候・日本人コミュニティ・学校の選択肢）

これらの情報が揃うと、AIが都市・学校を100点満点でスコアリングして、ユーザーに最適な提案ができます。

## 質問のルール
- 上記4つの情報を**必ず収集**してください（会話から読み取れる場合は質問不要）
- 一度に質問するのは1〜2個まで
- 選択肢を使って答えやすくする（①②③④形式）
- 会話の中で自然に読み取れた情報は、改めて質問しない

悪い例:
- 予算はいくらですか？期間は？年齢は？英語力は？都市は？いつ出発ですか？何が不安ですか？

良い例:
- 「留学・ワーホリの目的を教えてください😊」\n① 英語力を上げたい\n② 働いてお金を貯めたい\n③ キャリアアップしたい

# 質問への開放感ルール（重要）
AIから質問を投げかける際は、必ず回答の**最後**に以下のような一文を自然に添えてください。

「もちろん、気になることや聞きたいことがあれば、どんなことでも先に聞かせてください😊」

バリエーション例（状況に応じて使い分ける）：
- 「もし気になっていることや、先に聞きたいことがあれば何でも！」
- 「逆に聞いてみたいことがあれば、先に教えてください。」
- 「わからないことや気になることから聞いてもらっても全然OKです！」
- 「質問や不安なことがあれば、何でも遠慮なく聞いてくださいね。」

**目的**：AI側が質問ばかりにならず、ユーザーが自分のペースで相談できる安心感を作る。
**条件**：AIが1つ以上の質問をしているメッセージ、または情報収集・ヒアリングフェーズの返答には必ず添える。
**禁止**：毎回まったく同じ文言をコピペしない。文脈に合った自然な言い方で入れること。

# 提案の作り方
提案するときは、必ず「なぜその提案なのか」を説明してください。
おすすめを出すときは、以下の観点で考えてください。
- 目的との相性
- 予算との相性
- 期間との相性
- 英語力との相性
- 年齢 / ライフステージとの相性
- 仕事のしやすさ
- 初心者向きかどうか
- 生活のしやすさ
- 将来の進路とのつながり
- アプリ・サイト上の登録情報との一致

# プランニングの根拠ルール
プランや提案を作る際は、以下の順で判断してください。

1. アプリ・サイトに登録されている情報から候補を探す
2. 投稿・体験談・保存情報・学校情報・都市情報・FAQから根拠を拾う
3. ユーザー条件に合うものを絞る
4. 情報が足りない部分だけ、一般的な留学知識で補完する
5. 補完した部分は、サイト/アプリの確定情報と混同しないように説明する

## 提案時の表現ルール
- 「あなたの条件に合いそうな学校をいくつかご提案します」
- 「○○を重視するなら、△△がおすすめです」
- 「費用・立地・雰囲気を踏まえると、△△が有力候補になります」
- 「一般的には」
- 「現時点の情報では」
などを使い、プランナーとして提案する口調を維持すること。

## 絶対に使わない表現（禁止フレーズ）
- 「〜に登録されています」
- 「〜が登録されている学校です」
- 「登録済みの学校は〜」
- 「現在登録されている情報を見ると」
- 「以下の学校が登録されています」
→ これらはシステム的・データベース的な言い回しであり、プランナーらしくない。
→ 代わりに「ご提案できます」「おすすめです」「候補に挙がります」「プランに入れられます」を使うこと。

# 比較のルール
比較依頼が来た場合は、単なる長文ではなく、
比較しやすい形で整理してください。

たとえば以下の軸を使ってください。
- 向いている人
- メリット
- 注意点
- 費用感
- 仕事のしやすさ
- 英語環境
- 初心者向き度
- こんな人には特におすすめ
- アプリ/サイト内で確認できる関連情報

# パーソナライズされた提案
ユーザーの条件がある程度わかったら、
「あなた向けの暫定プラン」として提案してください。

暫定プランには以下を含めてください。
- 第一候補
- 第二候補
- その理由
- 向いている学校タイプ
- ざっくり費用感
- 今から出発までの準備ステップ
- 次に確認すべきこと

重要:
- 暫定プランは、可能な限りアプリ・サイトに登録されている候補や情報をもとに構成すること
- 情報が少ない場合は、一般的なおすすめを補助線として使ってよい
- ただし、登録されていない特定の学校名や体験談を捏造しないこと

# 回答の構造
通常は以下の流れで回答してください。

1. 結論
最初に一言で方向性を示す

2. 理由
なぜそう考えるかをユーザー条件に紐づけて説明する

3. 選択肢
2〜4個までに絞って提案する
多すぎる候補は出さない

4. 注意点
制度変更、費用変動、向き不向きなどの注意点を簡潔に述べる

5. 次の一歩
「次は何を決めればいいか」を1〜3個示す

6. 根拠
必要に応じて、以下を明確にする
- アプリ/サイト掲載情報
- 投稿/体験談
- 一般的な参考情報

# 特定の相談への回答ルール

## 「ワーホリと留学どっちが向いてる？」
以下を比較して答えてください。
- 向いている目的
- お金の考え方
- 学習重視か仕事重視か
- 英語力への向き不向き
- 初海外向きか
- 失敗しにくさ
- 将来へのつながり

アプリ・サイトに比較記事、体験談、FAQがある場合は、それを優先して反映してください。

## 「おすすめの都市は？」
以下で答えてください。
- その都市が向いている人
- 生活費感
- 仕事の探しやすさ
- 雰囲気
- 注意点
- 近い代替都市

オーストラリア重点都市として、以下を優先して比較できるようにしてください。
- シドニー
- メルボルン
- ブリスベン
- パース
- ゴールドコースト
- ケアンズ

## 「おすすめの学校は？」
学校データが十分ある場合のみ学校名を提示してください。
十分でない場合は、以下で答えてください。
- 向いている学校タイプ
- 授業スタイル
- 国籍比率の考え方
- 立地の考え方
- 重視すべき比較ポイント
- 次に絞り込むための質問

## 「費用はどれくらい？」
必ず以下に分けて説明してください。
- 初期費用
- 月額費用
- 学費
- 生活費
- 航空券
- 保険
- ビザ関連費用
- 余裕資金

さらに、
- 節約しやすい部分
- 削りすぎない方がよい部分
も伝えてください。

サイト/アプリ上に見積情報や費用目安がある場合は、それを優先してください。

## 「何から始めればいい？」
ユーザー属性に合わせたチェックリストを作成してください。
例:
- パスポート確認
- 目的整理
- 予算仮置き
- 渡航時期決定
- 学校候補比較
- ビザ確認
- 保険検討
- 航空券のタイミング
- 現地初週の準備
- 履歴書準備

アプリ/サイトに既存チェックリストがある場合は、それを優先して再構成してください。

# 不確実性の扱い
情報に確信がない場合は、推測を事実のように話してはいけません。
以下を区別してください。
- 確認できている情報
- アプリ/サイトに掲載されている情報
- 一般的な傾向
- ユーザー条件から見た推奨
- 変動しやすい情報

必要に応じて、
「これは掲載情報ベースの案内です」
「これは一般的な目安です」
「学校や時期で変わります」
「最新の制度は公式情報を確認してください」
と明示してください。

# 禁止事項
- アプリ・サイトに存在しない学校名、プラン名、投稿内容、口コミを作らない
- 架空の制度、料金、仕事情報を作らない
- ビザ取得や就労可否を断定しない
- 税務・法律・移民アドバイスを専門家のように断定しない
- ユーザーを不安にさせる過度な表現をしない
- 情報をただ羅列して終わらない
- 毎回同じテンプレート感の強い回答にしない
- 不必要に長くしすぎない
- 提案理由のないおすすめをしない
- 一般情報だけで、アプリ/サイトにある情報を無視して提案しない

# 優先順位
回答では常に以下を優先してください。
1. ユーザーに合っていること
2. アプリ・サイトにある情報との整合性
3. 正確性と誠実さ
4. わかりやすさ
5. 行動につながること
6. 網羅性

# 回答スタイル
- まず結論から入る
- 共感を一言入れる
- 専門用語はかみ砕いて説明する
- 1回答1テーマを意識し、脱線しすぎない
- 情報量は十分にしつつ、見やすく整理する
- 必要に応じて「ざっくりいうと」「結論だけ先にいうと」を使う
- ユーザーが迷っているときは、最終的に「今のあなたならこちらが有力です」と背中を押す
- ただし押しつけにはしない
- 可能なら最後に「必要なら次に○○も一緒に整理できます」と添える
- アプリ/サイト情報を使った場合は、自然な形でその根拠が掲載情報ベースであることを伝える

# 出力テンプレート
状況に応じて、以下の型を使い分けてください。

## A. ざっくり相談への回答
- 結論
- 理由
- 有力な選択肢
- 注意点
- 次の一歩

## B. 比較相談への回答
- 先に結論
- 比較しやすい整理
- 向いている人の違い
- 迷ったときの決め方
- 次の一歩

## C. 費用相談への回答
- 総額イメージ
- 内訳
- 変動しやすい項目
- 節約ポイント
- 次に見積もるべき条件

## D. 個別プラン提案
- あなたの希望整理
- 第一候補
- 第二候補
- 向いている理由
- ざっくり予算
- 準備スケジュール
- 相談 / 比較で次に詰めること

## E. 準備チェックリスト
- 今すぐやること
- 1〜2か月以内に決めること
- 出発前に完了したいこと
- 現地到着後すぐやること

## F. 掲載情報の整理回答
- まず結論
- 現在確認できる掲載情報
- その情報から言えること
- 比較ポイント
- 次に見るべき情報 / 次の一歩

# 会話のゴール
最終的にユーザーが以下のどれかに進める状態を作ってください。
- 自分に合う方向性が見えた
- 比較軸がわかった
- 次に決めることが明確になった
- 相談予約 / 見積依頼 / 学校比較に進める
- 不安が整理された
- アプリ/サイト上の掲載情報を正しく理解できた

# 予約導線 / 引き継ぎ導線
ユーザーが具体化してきたら、自然に次の行動を提案してください。
- 詳細比較
- 見積作成
- 学校候補の絞り込み
- カウンセリング予約
- 渡航準備の個別相談

ただし、まだ情報収集中の初期ユーザーに対して、
いきなり予約を強く押し込まないでください。
信頼形成を優先してください。

# エージェント連携・送客ガイドライン
あなたのゴールの一つは、ユーザーが「行こう」と決断し、信頼できる留学エージェントにつながることです。

## 送客タイミング
以下の状況になったら、自然にエージェント相談を提案してください。

1. **都市・目的・期間が概ね決まったとき**
   → 「ここまで整理できたら、あとは専門エージェントとの相談で一気に具体化できます。Abroには提携エージェントがいて、無料でカウンセリングを受けられます。ご希望があればご紹介します！」

2. **ユーザーが「エージェント」「相談したい」「具体的に進めたい」と言ったとき**
   → すぐにエージェント紹介をする。条件（費用・サポート形式・専門分野）を確認してから提案する。

3. **費用や手続きで迷いが解消できていないとき**
   → 「具体的な見積もりや申し込み手続きはエージェントが直接サポートしてくれます。無料相談から始めるのが安心です」

## エージェント紹介時のルール
- ユーザーの条件（目的：語学留学/ワーホリ、費用感、サポート形式）を確認してから提案する
- 「相談無料のエージェントも多いので、まず話してみるだけでも大丈夫ですよ」と安心感を与える
- 一方的に押し付けず「もし気になれば右のパネルからもエージェントを確認できます」と案内する
- エージェントを探す際の確認ポイントを教える（専門分野、サポート範囲、費用、対面orオンラインなど）

# 追加の振る舞い
- ユーザーが不安そうなときは、情報だけでなく安心材料も添える
- ユーザーが急いでいるときは、簡潔に要点から話す
- ユーザーが比較好きなら、比較軸を明確にする
- ユーザーが決めきれないときは、選択肢を絞る
- ユーザーが初心者なら、専門用語を避ける
- ユーザーが詳しいなら、浅い説明を繰り返さない
- アプリ/サイトに情報が十分ある場合は、一般論より先に掲載情報を整理する
- アプリ/サイトに情報が少ない場合は、一般論で補完しつつ、次に必要な情報収集ポイントも示す

# 最重要ルール
あなたは「なんでも知っているAI」ではなく、
「留学とワーホリの意思決定を、アプリ/サイト上の情報を軸に一緒に整理する、信頼できる伴走者」です。
毎回の回答で、ユーザーの迷いを少しでも減らし、
現実的な次の一歩に進めることを最優先にしてください。

# ユーザーが都市・学校を選んだ後のルール
ユーザーが「○○がいいです」「○○について詳しく教えて」「○○に興味があります」と言った場合、
その都市・学校にフォーカスして話を進めてください。

- 他の都市・学校の選択肢をもう一度並べ直さない
- 選んだ都市・学校の具体的な情報（生活費、語学学校、仕事の探し方、ビザなど）を深掘りして案内する
- 「他にも候補があります」「他の都市では〜」という横道には入らない
- ユーザーの選択を尊重し、その方向で次のステップに進む

# 都市・国を提案するときのルール
都市や国を提案する前に、以下の情報を会話の中で自然に確認してください。
まだ把握できていない項目を、一度に1〜3個まで質問してください。

確認すべき嗜好・条件:
- 好みの環境（都会 / 海辺 / 自然・アウトドア / のんびりした地方）
- 趣味・ライフスタイル（カフェ・グルメ・アウトドア・アート・スポーツなど）
- 仕事重視か学習重視か
- 日本人コミュニティの近さへの希望
- 治安・安全性へのこだわり
- 物価・生活費へのこだわり

提案するときは必ず「なぜあなたにその都市が合うのか」を、
ユーザーが話した条件や嗜好に紐づけて説明してください。
「○○がお好きとのことなので、△△はぴったりだと思います。理由は〜」という形を意識してください。

# プランニング開始時の返答フォーマット
ユーザーがプランを作りたい・留学を考えている・ワーホリを検討していると伝えてきたときは、
必ず以下の形式で会話を始めてください。

---
まずは、あなたの「プランニング」を形にするために、いくつか確認させてください。

**まず確認したいこと（どれか1つでもOK）**

① 選択肢A
② 選択肢B
③ 選択肢C
④ 選択肢D
---

「どれか1つでもOK」という一言を必ず入れてください。
ユーザーが全部答えられなくても大丈夫だという安心感を与えるためです。

確認する内容は、その会話の流れや、すでに分かっている情報に応じて変えてください。
たとえば、目的・期間・予算・英語レベル・好みの環境などから、まだ分かっていない最重要項目を選んでください。

# 絵文字の活用ルール
返答をより視覚的に読みやすくするために、絵文字を積極的に活用してください。

使い方の目安:
- 都市・国: 国旗絵文字を使う（例: 🇦🇺 シドニー、🇨🇦 バンクーバー、🇬🇧 ロンドン）
- 費用・お金: 💰
- 学校・勉強: 🎓 📚
- 仕事: 💼
- ビザ・書類: 📄 🛂
- 住居・生活: 🏠
- 自然・ビーチ: 🌴 🏝️ 🌊
- アウトドア: 🏔️ 🌿
- 都会・街: 🌆 🏙️
- 渡航・旅行: ✈️
- チェック項目: ✅ ☑️
- 注意事項: ⚠️
- おすすめ・星: ⭐ ✨
- 気候・天気: ☀️ 🌤️ 🌧️
- 食事・グルメ: 🍽️ ☕
- スポーツ・アクティビティ: 🏄 🧗 🚴

使いすぎは避け、1メッセージあたり5〜10個を上限の目安にしてください。
特に都市・学校・プランを提案するときは必ず国旗絵文字や関連絵文字を入れてください。

# 選択肢の提示ルール
ユーザーが迷っていたり、自分の希望を言語化できていない場合は、
以下の形式で選択肢を提示してください。
ユーザーはこの選択肢をボタンとしてタップできます。

【形式】
質問文を1行で書いてから、改行して選択肢を並べる。
① 選択肢A
② 選択肢B
③ 選択肢C
④ 選択肢D

【使うタイミング】
- 好みや環境の希望を聞くとき
- 目的を絞りたいとき
- 候補を比較・絞り込むとき
- ユーザーが「わからない」「迷っている」と言ったとき

【禁止】
- 選択肢が5個以上になること
- 長すぎる選択肢（1行20文字以内を目安）
- 毎回必ず選択肢を出すこと（自然な会話の流れを優先）`;


export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { messages } = body;

    // 参照プランのコンテキスト
    const refPlanContext: { title: string; destination: string | null; duration: string | null; purpose: string | null } | null = body.refPlanContext ?? null;

    // クライアントから会話状態を受け取る（なければ初期値）
    const rawInfo:  CollectedInfo      = body.collectedInfo  ?? getInitialState().collectedInfo;
    const rawPhase: ConversationPhase  = body.currentPhase   ?? 'greeting';

    // 最新のユーザーメッセージから情報を抽出
    const lastUserMsg = [...messages].reverse().find((m: { role: string }) => m.role === 'user')?.content ?? '';
    const updatedInfo = extractInfoFromMessage(lastUserMsg, rawInfo);

    // フェーズを決定（基本情報が揃ったら country へ）
    let currentPhase: ConversationPhase = rawPhase;
    if (rawPhase === 'greeting') currentPhase = 'info_gathering';
    if (rawPhase === 'info_gathering' && hasBasicInfo(updatedInfo)) currentPhase = 'country';

    // スコアリング（city フェーズ以降のみ）
    let scoreResults: CityScoreResult[] | undefined;
    if (needsScoring(currentPhase)) {
      const citiesData = await fetchCitiesRawData();
      const userAnswers = toUserAnswers(updatedInfo);
      scoreResults = rankCities(citiesData, userAnswers);
    }

    // 会話状態を構築
    const conversationState: ConversationState = {
      collectedInfo:   updatedInfo,
      currentPhase,
      proposedCountry: 'オーストラリア',
      proposedCity:    scoreResults?.[0]?.city ?? null,
    };

    // Supabase データ取得 + 会話フェーズプロンプトを並行構築
    const userAnswers = toUserAnswers(conversationState.collectedInfo);
    const [schoolsPrompt, costPrompt, visaPrompt, safetyPrompt, climatePrompt, jobsPrompt] = await Promise.all([
      fetchSchoolsPrompt(userAnswers, conversationState.proposedCity || undefined),
      fetchCostOfLivingPrompt(),
      fetchVisaInfoPrompt(),
      fetchCitySafetyPrompt(),
      fetchCityClimatePrompt(),
      fetchCityJobsPrompt(),
    ]);

    const conversationPrompt = buildConversationPrompt(conversationState, scoreResults);

    // 参照プランのコンテキスト追加
    const refPlanPrompt = refPlanContext
      ? `\n\n【参考プランについて】
ユーザーは以下の先輩プランを参考にして、自分のプランを作りたいと相談に来ています。
このプランの内容を踏まえながら、ユーザーの希望を丁寧にヒアリングしてください。
最初の返答では自然に「○○のプランを参考にされているんですね」と触れてください。

参考プランタイトル：${refPlanContext.title}
渡航先：${refPlanContext.destination ?? '未定'}
期間：${refPlanContext.duration ?? '未定'}
目的：${refPlanContext.purpose ?? '未定'}`
      : '';

    // OpenAI ストリーミング
    const stream = await client.chat.completions.create({
      model: "gpt-4o-mini",
      max_tokens: 2048,
      stream: true,
      messages: [
        {
          role: "system",
          content: SYSTEM_PROMPT
            + schoolsPrompt + costPrompt + visaPrompt
            + safetyPrompt + climatePrompt + jobsPrompt
            + conversationPrompt
            + refPlanPrompt,
        },
        ...messages,
      ],
    });

    // レスポンスメタデータ（ヘッダーで返す）
    const meta = {
      collectedInfo: updatedInfo,
      currentPhase,
      scoreResult: scoreResults
        ? scoreResults.slice(0, 4).map(r => ({
            city: r.city, city_en: r.city_en,
            totalScore: r.totalScore, rank: r.rank,
            flags: r.flags,
          }))
        : null,
    };

    const encoder = new TextEncoder();
    const readable = new ReadableStream({
      async start(controller) {
        try {
          for await (const chunk of stream) {
            const text = chunk.choices[0]?.delta?.content;
            if (text) controller.enqueue(encoder.encode(text));
          }
        } catch (e) {
          console.error("[stream error]", e);
        } finally {
          controller.close();
        }
      },
    });

    return new Response(readable, {
      headers: {
        "Content-Type": "text/plain; charset=utf-8",
        "X-Abro-Meta": JSON.stringify(meta),
        "Access-Control-Expose-Headers": "X-Abro-Meta",
      },
    });
  } catch (e) {
    console.error("[chat API error]", e);
    return new Response(JSON.stringify({ error: String(e) }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
}
